              ...
              <div key={idx} className="text-xs">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    {citation.source_homepage && (
                      <div className="text-gray-600 mt-1">
                        出典:{' '}
                        <a
                          href={citation.source_homepage}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-blue-600 hover:text-blue-800 hover:underline"
                        >
                          {citation.source_name || citation.source_homepage} ホームページ
                        </a>
                      </div>
                    )}
                  </div>
...
export default function Chat({ messages, onSendMessage, isLoading, conversationMode = 'multi', hasSentFirstMessage = false, onSwitchToMultiTurn, onFeedback }) {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        >
          {messages.length === 0 && (
            <div className="min-h-full flex items-center justify-center px-2">
              <div className="text-center text-gray-500">
                ...
                <h2 className="text-lg sm:text-xl font-semibold mb-2 text-gray-800">Welcome!</h2>
                <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Ask me anything about living in Japan.</p>
                <div className="text-left max-w-md mx-auto space-y-2">
                  <p className="text-xs sm:text-sm font-semibold text-gray-700">Try asking:</p>
                  <p className="text-xs sm:text-sm text-gray-600">• "How do I renew my work visa in Tokyo?"</p>
                  <p className="text-xs sm:text-sm text-gray-600">• "I need to change my visa status to permanent resident"</p>
                  <p className="text-xs sm:text-sm text-gray-600">• "Where can I get help with housing in Osaka?"</p>
                </div>
              </div>
            </div>
          )}
          ...
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {message.role === 'user' ? (
                <div className="max-w-[85%] sm:max-w-[70%] md:max-w-[60%] rounded-2xl px-4 py-3 text-white shadow-md bg-gradient-to-br from-zinc-700/95 to-zinc-600/95">
                  <div className="whitespace-pre-wrap break-words">{message.content}</div>
                </div>
              ) : (
                <div className="max-w-[95%] sm:max-w-[85%] md:max-w-[75%] px-1 py-0.5 text-gray-800">
      ...
      {/* Disclaimer */}
      <div className="px-3 py-1.5 text-center text-[10px] sm:text-xs text-amber-700 bg-amber-50 border border-amber-200">
        This is a research project demo (data collected ~Feb 2026). Information may be inaccurate or outdated.<br></br>Do not rely on this for legal or official decisions.
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-2 sm:p-3">
        <div className="relative">
          <textarea
            ...
            rows={3}
            className="w-full resize-none min-h-[70px] sm:min-h-[80px] max-h-48 px-3 sm:px-4 py-2.5 sm:py-3 pr-10 border border-gray-200 rounded-xl sm:rounded-2xl shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed text-sm sm:text-base"
          />
        </div>
        {conversationMode === 'single' && hasSentFirstMessage ? (
