"""Scope checker node for the Japan Procedures Agent.

Validates whether a user query falls within the supported topic area
(Japanese official procedures, visas, living in Japan) and detects
mid-conversation topic drift so the agent can warn the user.
"""
...
Assess the continuity:"""
    ...
    try:
        ...
        if result.continuity == "drift":
            logger.warning("Context drift detected")
...
def handle_context_drift(state: AgentState) -> dict:
    ...
    LangGraph pattern: Return state updates only.
    """
    logger.info("Generating context drift warning")
