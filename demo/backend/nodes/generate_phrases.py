"""Japanese vocabulary info agent.

Generates 3-5 key administrative terms (nouns, compound nouns, verb forms)
relevant to the current Q&A turn so the user knows the vocabulary they will
encounter at government offices.
...
def generate_useful_phrases(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not query_text or not answer_text:
            logger.info("Skipping phrase generation — no Q&A pair")
            return {}
        ...
        logger.info("Generating useful phrases")
...
"""
        ...
        for phrase in result.phrases[:5]:  # Limit to 5
            ...
            logger.info(f"  Phrase: {phrase.english}: {phrase.japanese}")
        ...
        logger.warning(
            "Phrase generation failed, but main answer will still be returned"
        )
        return {}
