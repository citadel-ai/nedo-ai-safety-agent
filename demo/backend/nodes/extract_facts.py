"""Fact-extraction info agent.

Analyses each Q&A turn to pull out facts about the *user's* situation
(visa expiry, family status, language ability, etc.) rather than
procedural details.  Fact keys are generated dynamically by the LLM.
Resilient: returns an empty dict on failure so the main answer is unaffected.
"""
...
def extract_facts_from_conversation(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not query_text or not answer_text:
            logger.info("Skipping fact extraction — no Q&A pair")
            return {}
        ...
        logger.info(f"Existing facts: {len(existing_facts)}")
...
"""
        ...
        for fact in extracted.facts:
            if fact.confidence in ["high", "medium"]:
                ...
                logger.info(
                    f"  New fact: {fact.key}: {fact.value} (confidence: {fact.confidence})"
                )
            else:
                logger.info(f"  Skipping low-confidence fact: {fact.key}")

        if facts_to_remove:
            logger.info(f"  Facts to remove: {facts_to_remove}")
        ...
        if new_facts:
            ...
            logger.info("No new user facts extracted")
            return {}

    except Exception as e:
        logger.error(f"Error extracting facts: {e}")
        logger.warning("Fact extraction failed, but main answer will still be returned")
        return {}
