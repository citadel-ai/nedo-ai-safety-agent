...
Bridges the FastAPI layer and the LangGraph agent: configures Langfuse
tracing, invokes the graph, and shapes the response for the frontend.
Also handles user feedback submission and thread state inspection.
"""
...
def query_agent(question: str, thread_id: str, conversation_mode: str = None) -> dict:
    ...
    if langfuse_handler:
        ...
        if trace_id:
            ...
            logger.warning("Could not extract trace_id from Langfuse handler")
...
def submit_feedback(trace_id: str, value: int, comment: str | None = None) -> dict:
    """
    ...
    try:
        ...
        langfuse_client.flush()  # Ensure score is sent before response

        logger.info(
            f"Feedback recorded for trace {trace_id}: "
...
def remove_collected_fact(thread_id: str, fact_key: str) -> dict:
    ...
    try:
        ...
        if fact_key not in collected_facts:
            logger.warning(
                f"Fact key '{fact_key}' not found in {list(collected_facts.keys())}"
            )
            return {
        ...
        logger.info(
            f"Removed fact '{fact_key}' from thread {thread_id}"
        )
