...
def create_working_workflow():
    ...
    def route_after_adversarial(state: JapanHelpdeskState) -> Literal["intake_agent", "END"]:
    ...
    def route_after_intake(state: JapanHelpdeskState) -> Literal["query_synthesizer", "END"]:
        """Route after intake.
    ...
    def route_after_scope(state: JapanHelpdeskState) -> Literal["agentic_search", "END"]:
...
class WorkingJapanHelpdeskAgent:
    ...
    async def process_query(
        ...
        try:
            ...
            logger.info(f"🟢 WORKING AGENT - final_response from workflow: '{final_response}'")
            ...
            if not final_response or len(final_response.strip()) < 10:
                ...
                logger.error(f"🔴 Completed steps: {result_state.get('completed_steps', [])}")
                ...
                if "intake" in current_step or not result_state.get("intake_session"):
                    ...
                    final_response = create_detailed_error_response(result_state, "intake")
                    
                elif "search" in current_step:
                    logger.error("🔴 SEARCH FAILURE DIAGNOSIS:")
                    logger.error(diagnose_search_failure(result_state))
                    final_response = create_detailed_error_response(result_state, "search")
                    
                else:
                    # Generic but informative error
                    logger.error("🔴 UNKNOWN FAILURE POINT")
                    final_response = create_detailed_error_response(result_state, current_step)
                
                # Log full state for debugging
                logger.error(f"🔴 FULL STATE DUMP:")
                for key, value in result_state.items():
                    if key not in ["_raw_vector_results", "_raw_google_results"]:  # Skip large data
                        logger.error(f"   {key}: {str(value)[:200]}")
            
            logger.info(f"🟢 WORKING AGENT - Final response to send: length={len(final_response)}, text='{final_response[:100]}...'")
            ...
            if intake_session and hasattr(intake_session, 'suggested_answers'):
                ...
                logger.info(f"🔵 WORKING AGENT - Extracted {len(suggested_answers)} suggested answers from intake_session")
            ...
            response = {
                ...
                "metadata": {
                    ...
                    "langfuse_trace_id": None,
                    "diagnostics": diagnostics.get_failure_report() if diagnostics.diagnostics else None
                }
            }
