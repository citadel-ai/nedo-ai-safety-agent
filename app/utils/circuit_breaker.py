...
class CircuitBreaker:
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == CircuitState.OPEN:
            ...
            if self._should_attempt_reset():
                ...
                # Reject call
                time_remaining = self.config.timeout - (time.time() - self.last_failure_time)
                logger.error(f"⛔ Circuit breaker '{self.name}' OPEN - rejecting call (retry in {time_remaining:.1f}s)")
    ...
    def _on_success(self):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.success_count >= self.config.success_threshold:
                logger.info(f"🟢 Circuit breaker '{self.name}' CLOSED - system recovered")
                self.state = CircuitState.CLOSED
                self.failure_count = 0
                self.success_count = 0
        else:
    ...
    def _on_failure(self):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            logger.error(f"🔴 Circuit breaker '{self.name}' OPEN - failure threshold reached")
            self.state = CircuitState.OPEN
