...
class WorkflowDiagnostics:
    ...
    def _detect_state_changes(self, before: Dict[str, Any], after: Dict[str, Any]) -> Dict[str, str]:
        """Detect what changed in state."""
        changes = {}
        
        # Check key fields
        important_fields = [
            "final_response", "intake_session", "synthesized_search_query",
            "hybrid_results", "recommendations", "errors"
        ]
        ...
        for field in important_fields:
            ...
            if before_val != after_val:
                changes[field] = f"{self._summarize_value(before_val)} → {self._summarize_value(after_val)}"
...
def diagnose_intake_failure(state: Dict[str, Any]) -> str:
    ...
    if not intake_session:
        ...
        if not hasattr(intake_session, 'next_questions'):
            ...
            issues.append(f"   is_complete: {getattr(intake_session, 'is_complete', 'N/A')}")
            issues.append(f"   conversation_history: {len(getattr(intake_session, 'conversation_history', []))} messages")
            issues.append(f"   required_context: {getattr(intake_session, 'required_context_fields', [])}")
            issues.append(f"   missing_context: {getattr(intake_session, 'missing_context_fields', [])}")
...
def diagnose_llm_truncation(response_content: str, max_expected: int = 1500) -> Optional[str]:
    """Diagnose if LLM response was truncated."""
    
    if len(response_content) < 100:
        return f"⚠️ Response very short ({len(response_content)} chars) - may be truncated"
    
    # Check if JSON is incomplete
    if "```json" in response_content:
        if not response_content.strip().endswith("```") and not response_content.strip().endswith("}"):
...
def create_detailed_error_response(
    ...
    response_parts.append(f"**Completed Steps**: {', '.join(completed_steps) if completed_steps else 'None'}")
    ...
    if original_error:
        response_parts.append("")
        response_parts.append(f"**Technical Error**: {type(original_error).__name__}: {str(original_error)}")
...
def validate_state_integrity(state: Dict[str, Any]) -> List[str]:
    ...
    required_fields = [
        "user_input", "user_id", "session_id", "current_step",
        "completed_steps", "errors", "final_response"
    ]
    ...
    if "completed_steps" in state and not isinstance(state["completed_steps"], list):
        issues.append(f"completed_steps should be list, got {type(state['completed_steps'])}")
