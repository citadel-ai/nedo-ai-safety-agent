...
async def adversarial_detector_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    try:
        ...
        logger.info(f"🛡️ ADV DETECTOR - raw LLM preview: {raw[:300]}{'...' if len(raw) > 300 else ''}")
        ...
        try:
            ...
            logger.warning(f"🟡 ADV DETECTOR - Direct parse failed, extracting JSON: {parse_err}")
            try:
        ...
        logger.info(f"🛡️ ADV DETECTOR - parsed: is_adversarial={result.is_adversarial}, threat_type={result.threat_type}, confidence={result.confidence}, reason={result.reason}")
        ...
        if result.is_adversarial:
            ...
            state["final_response"] = f"I cannot process this request. Reason: {result.reason}"
