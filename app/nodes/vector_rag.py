...
async def vector_rag_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    try:
        ...
        if vector_results:
            vector_results = sorted(vector_results, key=lambda r: r.similarity_score, reverse=True)[:3]
        ...
        if vector_results:
            ...
            for idx, r in enumerate(vector_results, start=1):
                ...
                citations.append(f"[{idx}] {snippet} (Source: {r.source}, Score: {r.similarity_score:.2f})")
            summary = "\n".join(citations)
            confidence = sum(r.similarity_score for r in vector_results) / len(vector_results)
        ...
        result = MergedSearchResult(
            ...
            sources=sources,
            recommendations=["Verify with official sources", "Contact relevant government office"]
        )
