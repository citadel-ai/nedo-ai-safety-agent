...
class ProcedureStep(BaseModel):
    ...
    location: str = Field(default="", description="Where to do this (office, online, etc.)")
    required_documents: List[str] = Field(default_factory=list, description="Documents needed for this step")
    ...
    dependencies: List[int] = Field(default_factory=list, description="Step numbers that must be completed first")
    tips: List[str] = Field(default_factory=list, description="Helpful tips or common mistakes")
...
class MultiStepProcedure(BaseModel):
    ...
    steps: List[ProcedureStep] = Field(default_factory=list, description="Ordered list of steps")
    important_notes: List[str] = Field(default_factory=list, description="Critical information to know")
    common_mistakes: List[str] = Field(default_factory=list, description="What people often do wrong")
...
async def multi_step_procedure_agent_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    """
    ...
    try:
        ...
        if state.get("hybrid_results"):
            ...
            search_summary = f"Found {vector_count} vector results and {google_count} Google results"
        ...
        if state.get("_raw_vector_results"):
            logger.info(f"📋 Available vector results: {len(state['_raw_vector_results'])}")
        ...
        if state.get("_raw_google_results"):
            logger.info(f"📋 Available Google results: {len(state['_raw_google_results'])}")
            for i, res in enumerate(state["_raw_google_results"][:2], 1):
                ...
                if isinstance(res, str):
                    ...
                    if content_attr:
                        content = content_attr() if callable(content_attr) else content_attr
                        logger.info(f"   Google {i} content preview: {str(content)[:150]}...")
                    else:
        ...
        messages = [
            SystemMessage(content="You are an expert at analyzing administrative procedures."),
        ...
        if procedure.is_multi_step and procedure.steps:
            logger.info(f"✅ MULTI-STEP PROCEDURE DETECTED: '{procedure.procedure_name}'")
            ...
            recommendations = []
            recommendations.append(f"**{procedure.procedure_name}** (Est. time: {procedure.total_estimated_time})")
            ...
            for step in procedure.steps:
                ...
                if step.required_documents:
                    recommendations.append(f"📄 Documents: {', '.join(step.required_documents)}")
        ...
        logger.info(f"⏱️ Multi-step procedure analysis completed in {processing_time:.2f}s")
