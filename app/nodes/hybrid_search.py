...
async def hybrid_search_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    if intake and getattr(intake, "collected_info", None):
        ...
        location = intake.collected_info.get("location") or getattr(intake, "user_location", None)
    ...
    try:
        ...
        google_task = asyncio.create_task(get_enhanced_search_results(query, num_results=5))
        ...
        ]) if vector_results else "No vector database results found."
        
        google_results_text = "\n".join([
            f"- {result}" for result in google_results
        ]) if google_results else "No Google search results found."
        ...
        # Create messages
        messages = [
            SystemMessage(content="You are a hybrid search agent combining multiple information sources."),
        ...
        fallback_result = MergedSearchResult(
            ...
            recommendations=["Try rephrasing your query", "Contact support if the issue persists", "Visit official government websites for the most current information"]
        )
