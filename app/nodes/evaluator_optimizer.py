...
class ResponseEvaluation(BaseModel):
    ...
    missing_elements: list[str] = Field(description="Important information that's missing")
    
    needs_improvement: bool = Field(description="Whether the response needs to be improved")
...
async def evaluate_response(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    try:
        ...
        user_context = {
            ...
            "user_location": getattr(intake_session, 'user_location', None) if intake_session else None,
            "visa_type": getattr(intake_session, 'visa_type', None) if intake_session else None,
            "timeline": getattr(intake_session, 'timeline', None) if intake_session else None,
            "urgency": getattr(intake_session, 'urgency_level', None) if intake_session else None,
        }
        ...
        state["agent_reasoning"].append(f"Response evaluated - Score: {evaluation.overall_score:.2f}")
...
async def optimize_response(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    try:
        ...
        messages = [
            SystemMessage(content="You are a response optimizer focused on helpfulness and accuracy."),
            HumanMessage(content=prompt)
        ]
        ...
        state["agent_reasoning"].append("Response optimized based on evaluation feedback")
        
        # Increase confidence score if optimization was successful
        if evaluation.overall_score < 0.8:
            state["confidence_score"] = min(1.0, state.get("confidence_score", 0.0) + 0.2)
...
async def evaluator_optimizer_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    if (state.get("response_evaluation") and 
        state["response_evaluation"].needs_improvement and 
        optimization_count < 2):  # Limit to 2 optimization rounds
