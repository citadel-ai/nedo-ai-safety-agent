...
class RealGoogleSearch:
    ...
    def __init__(self):
        logger.info(f"🔍 GOOGLE SEARCH INIT DEBUG - Initializing RealGoogleSearch:")
    ...
    async def search(
        ...
        logger.info(f"🔍 GOOGLE SEARCH DEBUG - Original Input:")
        ...
        logger.info(f"🔍 GOOGLE SEARCH DEBUG - Enhanced Query:")
        ...
        logger.info(f"🔍 GOOGLE SEARCH DEBUG - Available Methods:")
        ...
        for method_num, method in enumerate(search_methods, 1):
            try:
                logger.info(f"🔍 GOOGLE SEARCH DEBUG - Trying Method {method_num}: {method.__name__}")
                results = await method(enhanced_query, num_results)
                
                # Debug: Log results
                logger.info(f"🔍 GOOGLE SEARCH DEBUG - Method {method.__name__} Results:")
                logger.info(f"   Success: True")
                logger.info(f"   Result count: {len(results)}")
                for i, result in enumerate(results[:3], 1):  # Show first 3 results
                    logger.info(f"   Result {i}: {result[:100]}{'...' if len(result) > 100 else ''}")
                
                if results:
                    logger.info(f"✅ Google search successful with {method.__name__}: {len(results)} results")
                    return results
                else:
                ...
                logger.info(f"🔍 GOOGLE SEARCH DEBUG - Exception details: {type(e).__name__}: {str(e)}")
                continue
        
        # If all methods fail, return empty list
        logger.error("❌ All Google search methods failed")
        logger.info(f"🔍 GOOGLE SEARCH DEBUG - Final Output: Empty list")
        return []
    
    def _enhance_query_for_japan(self, query: str, site_filter: Optional[str] = None) -> str:
        ...
        if "japan" not in query.lower() and "japanese" not in query.lower() and "日本" not in query:
    ...
    async def _search_with_google_custom_search(self, query: str, num_results: int) -> List[str]:
        """Search using Google Custom Search API."""
        logger.info(f"🔍 CSE DEBUG - Checking API configuration:")
        logger.info(f"   GOOGLE_API_KEY set: {bool(self.google_api_key)}")
        logger.info(f"   GOOGLE_CSE_ID set: {bool(self.google_cse_id)}")
        logger.info(f"   GOOGLE_CSE_ID value: {self.google_cse_id}")
        ...
        logger.info(f"🔍 CSE DEBUG - API Request:")
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:
            async with session.get(url, params=params) as response:
                logger.info(f"🔍 CSE DEBUG - API Response:")
                ...
                logger.info(f"🔍 CSE DEBUG - Response Data:")
    ...
    async def _search_with_googlesearch_python(self, query: str, num_results: int) -> List[str]:
        """Search using googlesearch-python library (free but rate-limited)."""
        logger.info(f"🔍 GOOGLESEARCH-PYTHON DEBUG - Starting search:")
        logger.info(f"   Query: {query}")
        logger.info(f"   Num results: {num_results}")
        
        try:
            from googlesearch import search
            logger.info(f"🔍 GOOGLESEARCH-PYTHON DEBUG - Package imported successfully")
            
            # Run in thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            
            logger.info(f"🔍 GOOGLESEARCH-PYTHON DEBUG - Executing search...")
            ...
            urls = await loop.run_in_executor(
                None, 
                lambda: list(search(filtered_query, num_results=num_results, lang="en"))
            )
            
            logger.info(f"🔍 GOOGLESEARCH-PYTHON DEBUG - Search completed:")
            ...
            logger.info(f"🔍 GOOGLESEARCH-PYTHON DEBUG - Final snippets: {len(snippets)} items")
            return snippets
    ...
    async def _fallback_to_mock(self, query: str, num_results: int) -> List[str]:
        ...
        logger.info(f"🔍 MOCK FALLBACK DEBUG - Starting fallback:")
        logger.info(f"   Query: {query}")
        logger.info(f"   Num results: {num_results}")
        ...
        logger.info(f"🔍 MOCK FALLBACK DEBUG - Calling mock_google_search function")
        
        # Run mock search in thread pool
        loop = asyncio.get_event_loop()
        results = await loop.run_in_executor(None, mock_google_search, query)
        
        logger.info(f"🔍 MOCK FALLBACK DEBUG - Mock search results:")
        logger.info(f"   Raw results count: {len(results)}")
        for i, result in enumerate(results[:3], 1):  # Show first 3 results
            logger.info(f"   Mock result {i}: {result[:100]}{'...' if len(result) > 100 else ''}")
        
        final_results = results[:num_results]
        logger.info(f"🔍 MOCK FALLBACK DEBUG - Final results: {len(final_results)} items")
        return final_results


# Global instance
_google_search_instance: Optional[RealGoogleSearch] = None
...
async def real_google_search(query: str, num_results: int = 5) -> List[str]:
    ...
    logger.info(f"🔍 REAL_GOOGLE_SEARCH DEBUG - Entry point:")
    logger.info(f"   Query: '{query}'")
    logger.info(f"   Num results: {num_results}")
    
    search_engine = get_google_search()
    logger.info(f"🔍 REAL_GOOGLE_SEARCH DEBUG - Search engine initialized")
    
    results = await search_engine.search(query, num_results)
    
    logger.info(f"🔍 REAL_GOOGLE_SEARCH DEBUG - Final output:")
...
def get_search_config() -> Dict[str, Any]:
    ...
    search_engine = get_google_search()
    return {
        "google_cse_configured": bool(search_engine.google_api_key and search_engine.google_cse_id),
