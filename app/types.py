...
class AdversarialInputResult(BaseModel):
    ...
    sanitized_query: Optional[str] = Field(default=None, description="Cleaned version if applicable")

class IntakeSession(BaseModel):
    """Enhanced session state for intelligent intake agent."""
    session_id: str = Field(description="Unique session identifier")
    user_id: str = Field(description="User identifier")
    conversation_history: List[str] = Field(default_factory=list, description="Previous exchanges")
    conversation_summary: str = Field(default="", description="Rolling summary of the conversation for context")
    collected_info: Dict[str, Any] = Field(default_factory=dict, description="Information gathered")
    current_step: str = Field(default="initial", description="Current step in intake process")
    ...
    user_location: Optional[str] = Field(default=None, description="City/Prefecture in Japan")
    visa_type: Optional[str] = Field(default=None, description="Type of visa (Student, Work, Tourist, etc.)")
    current_status: Optional[str] = Field(default=None, description="Current visa/legal status")
    timeline: Optional[str] = Field(default=None, description="When they need this resolved")
    urgency_level: Optional[str] = Field(default=None, description="Low, Medium, High, Critical")
    language_preference: Optional[str] = Field(default=None, description="Preferred language for assistance")
    previous_attempts: Optional[str] = Field(default=None, description="What they've already tried")
    specific_office_needed: Optional[str] = Field(default=None, description="Specific government office they need")
    
    # Context analysis
    required_context_fields: List[str] = Field(default_factory=list, description="Context fields needed for this query")
    missing_context_fields: List[str] = Field(default_factory=list, description="Still missing context")
    context_completeness_score: float = Field(default=0.0, description="How complete the context is (0-1)")
    
    # Intelligent questioning
    next_questions: List[str] = Field(default_factory=list, description="Next questions to ask")
    question_priority: Dict[str, int] = Field(default_factory=dict, description="Priority of each question type")
    suggested_answers: List[str] = Field(default_factory=list, description="Quick-reply suggestions for the current question")
...
class MergedSearchResult(BaseModel):
    ...
    vector_results: List[VectorSearchResult] = Field(description="Results from vector database")
    ...
    confidence_score: float = Field(description="Overall confidence in the merged result")
...
class LegalAdviceCheck(BaseModel):
    ...
    contains_legal_advice: bool = Field(description="Whether response contains legal advice")
...
class AgentTodo(BaseModel):
    ...
    assigned_to: Optional[str] = Field(default=None, description="Which agent should handle this")
    dependencies: List[str] = Field(default_factory=list, description="TODO IDs this depends on")
    ...
    deadline: Optional[str] = Field(default=None, description="When this should be completed")
...
class AgentPlan(BaseModel):
    ...
    todos: List[AgentTodo] = Field(default_factory=list, description="List of tasks to complete")
    current_todo_id: Optional[str] = Field(default=None, description="Currently active TODO")
    plan_status: str = Field(default="active", description="active, completed, failed, paused")
    confidence_in_plan: float = Field(default=0.0, description="Agent's confidence in this plan (0-1)")
    alternative_plans: List[str] = Field(default_factory=list, description="Alternative approaches considered")
...
class JapanHelpdeskState(TypedDict):
    ...
    scope_check_result: Optional[ScopeCheckResult]  # Consistent key across nodes/routers
    vector_results: Optional[MergedSearchResult]
    hybrid_results: Optional[MergedSearchResult]
    rag_results: Optional[LegalResponse]
    legal_check_result: Optional[LegalAdviceCheck]  # Consistent key across nodes/routers
