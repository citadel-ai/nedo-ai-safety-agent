...
import time
from app.types import JapanHelpdeskState, AdversarialInputResult, ScopeCheckResult
...
async def minimal_scope_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    print(f"🎯 SCOPE NODE COMPLETED - Steps now: {len(updated_state['completed_steps'])}")
    return updated_state
...
async def minimal_response_node(state: JapanHelpdeskState) -> JapanHelpdeskState:
    ...
    print(f"📝 RESPONSE NODE COMPLETED - Steps now: {len(updated_state['completed_steps'])}")
    return updated_state
