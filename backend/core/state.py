...
class AgentState(MessagesState):
    ...
    answer: str = ""
    citations: Annotated[
        List[Dict], _replace_list
    ] = []  # Citation dicts with URL, title, pages - replaced on each turn
    error: Optional[str] = None

    # Info cards data - replaced on follow-ups (not appended)
    useful_phrases: Annotated[
        List[Dict], _replace_list
    ] = []  # {"japanese": str, "romaji": str, "english": str}
    useful_places: Annotated[
        List[Dict], _replace_list
    ] = []  # {"name": str, "address": str, "place_id": str, "maps_url": str}
