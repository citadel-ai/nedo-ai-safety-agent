...
from ..nodes.check_scope import check_query_scope, handle_out_of_scope, handle_context_drift
...
def create_graph():
    ...
    workflow.add_node("context_drift", handle_context_drift)  # New: context drift handler
    workflow.add_node("route_search", lambda state: state)  # Dummy node for search method routing
    workflow.add_node("search", search_and_respond)  # Original search method
    workflow.add_node("search_answer", search_and_respond_with_answer)  # New: answer method
