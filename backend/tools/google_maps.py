"""
Google Maps search tool using Google GenAI SDK with Maps grounding.

This wraps the google.genai.Client to provide a standard LangChain tool interface
while leveraging Google Maps grounding for real place data.

IMPORTANT: Must use us-central1 region for Google Maps grounding to return results.
"""
...
class GoogleMapsSearchInput(BaseModel):
    ...
    query: str = Field(description="What to search for (e.g., 'immigration offices', 'city halls')")
...
class GoogleMapsSearchTool(BaseTool):
    ...
    description: str = "Search for real places using Google Maps. Returns place names, IDs, and links."
    ...
    def _run(self, query: str, location: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"🗺️ Google Maps search: '{query}' near '{location}'")
            ...
            response = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    tools=[types.Tool(google_maps=types.GoogleMaps(enable_widget=False))]
                ),
            )
    ...
    def _extract_places_from_response(self, response) -> List[Dict[str, Any]]:
        ...
        if not hasattr(candidate, 'grounding_metadata') or not candidate.grounding_metadata:
            logger.warning("   ⚠️ No grounding_metadata in candidate")
            return places
        
        grounding_meta = candidate.grounding_metadata
        
        # Check for grounding chunks
        if not hasattr(grounding_meta, 'grounding_chunks') or not grounding_meta.grounding_chunks:
            logger.warning("   ⚠️ No grounding_chunks in metadata")
            return places
    ...
    def _extract_addresses_from_supports(self, grounding_meta) -> Dict[int, str]:
        ...
        if not hasattr(grounding_meta, 'grounding_supports') or not grounding_meta.grounding_supports:
    ...
    def _extract_place_from_chunk(
        ...
        return {
            ...
            "address": address,
            "place_id": place_id,
            "maps_url": uri or f"https://www.google.com/maps/place/?q=place_id:{place_id}"
        }
