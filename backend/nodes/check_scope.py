...
class ScopeCheckResult(BaseModel):
    ...
    is_in_scope: bool = Field(description="Whether the query is about Japanese official procedures, visas, or living in Japan")
    reason: str = Field(description="Brief explanation of why this is in or out of scope")
...
class ContextCheckResult(BaseModel):
    ...
    previous_topic: str = Field(description="Brief description of what the previous conversation was about")
    new_topic: str = Field(description="Brief description of what the new query is about")
    reason: str = Field(description="Explanation of the continuity assessment")


@trace_node("check_query_scope")
def check_query_scope(state: AgentState) -> Literal["in_scope", "out_of_scope", "context_drift"]:
    """
    ...
    return _check_scope_with_context(query_text, messages[:-1], state.get("collected_facts", {}))
...
def _check_scope_with_context(
    ...
    facts_str = ", ".join([f"{k}: {v}" for k, v in collected_facts.items() if v]) if collected_facts else "None"
