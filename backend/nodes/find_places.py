...
class PlaceType(BaseModel):
    ...
    place_type: str = Field(description="Type of governmental facility (e.g., 'immigration offices', 'city halls')")
...
class PlaceTypes(BaseModel):
    ...
    types: List[PlaceType] = Field(description="1-2 types of governmental facilities to search for")
...
5. If no relevant government facilities are needed, return an empty list"""
        ...
        if len(all_places) < 2:
            ...
            search_term = result.types[0].place_type if result.types else "government office"
