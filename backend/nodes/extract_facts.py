...
class ExtractedFact(BaseModel):
    ...
    key: str = Field(description="Short descriptive key for this fact (e.g., 'Timeline', 'Family Situation', 'Language Ability')")
    value: str = Field(description="The actual information about the user")
    confidence: str = Field(description="'high' if directly stated, 'medium' if inferred, 'low' if uncertain")


class ExtractedFacts(BaseModel):
    """Collection of facts about the user with instructions for merging."""
    facts: List[ExtractedFact] = Field(description="List of facts extracted from this conversation turn")
    facts_to_remove: List[str] = Field(default=[], description="Keys of existing facts that should be removed (if contradicted or no longer relevant)")
...
def extract_facts_from_conversation(state: AgentState) -> AgentState:
    ...
    try:
        ...
        existing_facts_str = "\n".join([f"- {k}: {v}" for k, v in existing_facts.items()]) if existing_facts else "None yet"
