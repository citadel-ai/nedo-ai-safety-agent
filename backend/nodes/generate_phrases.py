...
class JapanesePhrase(BaseModel):
    ...
    japanese: str = Field(description="Japanese phrase in hiragana/katakana/kanji, WITHOUT romaji")
...
class UsefulPhrases(BaseModel):
    ...
    phrases: List[JapanesePhrase] = Field(description="List of 3-5 key nouns, compound nouns, and verb forms")
...
def generate_useful_phrases(state: AgentState) -> AgentState:
    ...
    try:
        ...
        context_info = f"User context: {visa_type} visa in {location}" if visa_type and location else ""
...
"""
        ...
        logger.info("⚠️  Phrase generation failed, but main answer will still be returned")
        return {}
