...
class FactsExtractor:
    ...
    def extract_from_session(self, session: IntakeSession) -> CollectedFacts:
        ...
        # Specific office
        if office := (
            collected.get("specific_office_needed") or session.specific_office_needed
        ):
            facts.specific_office = self._create_fact(
                "specific_office_needed", str(office)
            )
        ...
        # Language preference
        if lang := (
            collected.get("language_preference") or session.language_preference
        ):
            facts.language_preference = self._create_fact(
                "language_preference", str(lang)
            )

        # Previous attempts
        if attempts := (
            collected.get("previous_attempts") or session.previous_attempts
        ):
            facts.previous_attempts = self._create_fact(
                "previous_attempts", str(attempts)
            )

        return facts

    def _create_fact(
        self, key: str, value: str, confidence: str | None = None
    ) -> DisplayFact:
        """Create a DisplayFact with metadata."""
        metadata = self.FACT_METADATA.get(
            key, {"label": key.replace("_", " ").title(), "icon": None}
        )
