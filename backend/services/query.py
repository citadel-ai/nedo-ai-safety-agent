...
def query_agent(question: str, thread_id: str, conversation_mode: str = None) -> dict:
    ...
    if langfuse_handler:
        ...
        try:
            ...
            collected_facts = snapshot.values.get("collected_facts", {}) if snapshot and snapshot.values else {}
...
def remove_collected_fact(thread_id: str, fact_key: str) -> dict:
    ...
    try:
        ...
        if not snapshot or not snapshot.values:
            ...
            return {"error": f"Thread '{thread_id}' not found. Please start a new conversation."}
        ...
        if fact_key not in collected_facts:
            logger.warning(f"⚠️  Fact key '{fact_key}' not found in {list(collected_facts.keys())}")
            return {"error": f"Fact '{fact_key}' not found. Available facts: {list(collected_facts.keys())}"}
        ...
        logger.info(f"✅ Successfully removed fact '{fact_key}' from thread {thread_id}")
